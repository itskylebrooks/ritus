...
export default function GuideModal({ open, onClose, onLoadExample }: GuideModalProps) {
  ...
  return (
    <div className={`fixed inset-0 z-[60] flex items-center justify-center p-5 transition-colors duration-250 ${closing || entering ? 'bg-transparent' : 'bg-overlay backdrop-blur-sm'}`} onClick={()=>{ if(!closing) onClose(); }}>
      <div
      ...
      >
        <div className="absolute top-2 right-2">
          <button aria-label="Close guide" onClick={()=>{ if(!closing) onClose(); }} className="w-8 h-8 rounded-full flex items-center justify-center bg-control text-muted transition duration-200 hover-nonaccent">
            ✕
          </button>
        </div>
  ...
  <div className="mt-5 flex items-center justify-between text-xs text-muted">
      ...
      {STEPS.map((_,i)=> <span key={i} className={`h-1.5 w-1.5 rounded-full ${i===step? 'bg-accent':'bg-chip'}`}/>) }
          </div>
        </div>
        <div className="mt-6">
          {last ? (
            ...
            <div className="flex gap-3">
              <button onClick={async()=> { handleLoadClick() }} className="flex-1 rounded-md px-3 py-2 text-sm font-medium bg-control text-strong transition duration-200 hover-nonaccent">Load data</button>
