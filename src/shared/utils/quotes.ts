...
import { AUTHOR_QUOTES } from './quotes_author';

// Backwards-compatible export for the default pack (defined above in this file)
export const QUOTES = DEFAULT_QUOTES;

// Keep `QUOTES` (the default collection) as the default collection for backward compatibility
// and as the "default pack" (it intentionally excludes collectible packs).

// Map collectible ids to their corresponding packs. Add more packs here as files are added.
export const QUOTE_PACKS: Record<string, Quote[]> = {
  // collectible id -> array of quotes
  quotes_author: AUTHOR_QUOTES,
};

// Return the appropriate pool for a selected pack id. If no pack is selected, return the
// default pack (the in-file `QUOTES` array which excludes collectible packs). If a selected
// pack exists but is empty or unknown, fall back to the default.
export function getQuotePool(packId?: string): Quote[] {
  if (!packId) return QUOTES;
  const p = QUOTE_PACKS[packId];
