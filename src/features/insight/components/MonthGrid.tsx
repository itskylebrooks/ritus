...
export default function MonthGrid({
  ...
  const displayMonthLabels: (string | null)[] = [...Array(2).fill(null), ...monthLabels].slice(
    0,
    monthLabels.length,
  );
  ...
  return (
    <div
      ref={containerRef}
      className={`mt-3 ${allowScroll ? 'overflow-x-auto' : 'overflow-x-hidden'} overflow-y-hidden`}
    >
      <div className="mb-2 flex items-center gap-2">
        ...
        <div className="flex gap-1">
          {displayMonthLabels.map((lab, i) => (
            <div
              key={i}
              className="w-3 text-[11px] text-neutral-600 dark:text-neutral-300 text-center"
            >
              {lab}
            </div>
          ))}
      ...
      <div className="flex gap-3">
        ...
        <div className="flex flex-col items-end gap-1 mr-2">
          {weekdayLabels.map((d) => (
            <div
              key={d}
              className="h-3 text-[12px] text-neutral-600 dark:text-neutral-300 leading-3"
            >
              {d}
            </div>
          ))}
