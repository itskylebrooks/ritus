...
export default function EmojiHistoryCard({ ready = true }: { ready?: boolean }) {
  ...
  const Container: React.FC<{
    ...
    fillWidth?: boolean;
    baseYear?: number;
  }> = ({ cols, showMonthLabels, fillWidth, baseYear }) => {
    ...
    return (
      <div
        ref={scrollerRef}
        className={`mt-2 ${fillWidth ? 'overflow-y-hidden' : 'overflow-x-auto overflow-y-hidden'}`}
        style={fillWidth ? { overflowX: 'hidden' } : undefined}
      >
        {showMonthLabels && (
          ...
          <div className="flex flex-col items-end gap-1 mr-2">
            {weekdayLabels.map((d) => {
              const style = fillWidth
                ? { height: `calc(${cellVar})`, lineHeight: `calc(${cellVar})` }
                ...
                {col.map((d, rIdx) => {
                  ...
                  return (
                    <div
                      ...
                      style={
                        fillWidth
                          ? { height: `calc(${cellVar})`, width: `calc(${cellVar})` }
                      ...
                      {e ? (
                        ...
                        <span
                          ...
                          style={
                            fillWidth
                              ? {
                                  fontSize: 'calc(var(--cell, 14px) * 0.6)',
                                  lineHeight: `calc(${cellVar})`,
                                }
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-300">
          ...
          <div className="tabular-nums">
            {new Intl.DateTimeFormat('en', { year: 'numeric' }).format(yearDate)}
          </div>
          <button
            onClick={() => shiftYear(1)}
